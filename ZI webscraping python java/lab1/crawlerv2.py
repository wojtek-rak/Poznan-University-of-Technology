...
def getNormalisedURLs(retrievedURLs):
    retrievedURLs = [x.lower() for x in retrievedURLs]
...
def getFilteredURLs(c, retrievedURLs):
    ...
    toLeft = [url for url in retrievedURLs if url.lower().startswith(c.rootPage)]
    result = [url for url in toLeft if url != c.toFetch]
    toLeft = set(result)
    if c.debug:
